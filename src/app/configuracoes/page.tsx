  ...
  useEffect(() => {
    async function loadAll() {
      ...
      try {
        ...
        // 1. Preenche perfil imediatamente com dados do usuário autenticado
        setProfile(prev => ({
          ...prev,
        ...
        // 3. Busca org via server action (admin client, ignora RLS)
        const orgRes = await getMyOrgAction()
        if (orgRes.success && orgRes.data) {
          ...
          setOrg({
            ...orgData,
            type: meta.tipo || 'pj',
            document: orgData.document || '', // Prioriza a coluna oficial da tabela para o CNPJ
            address: meta.endereco || orgData.address || '',
            phone: meta.telefone || orgData.phone || '',
            website: meta.website || orgData.website || ''
          })

          // Map representative legal to profile
          if (meta.representante_legal) {
            const rep = meta.representante_legal as any
            setProfile(prev => ({
              ...prev,
              nome: rep.nome || prev.nome,
              email: user.email || '',
              cargo: rep.cargo || prev.cargo,
              telefone: (meta.telefone as string) || prev.telefone,
              cpf: rep.cpf || '', // CPF do titular separado
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8 text-center md:text-left">
                      <div className="relative group">
                        ...
                        <label 
                          ...
                          className="absolute -bottom-1 -right-1 md:bottom-1 md:right-1 w-8 h-8 md:w-10 md:h-10 rounded-xl md:rounded-2xl bg-primary text-white border-2 md:border-4 border-white shadow-lg flex items-center justify-center hover:scale-110 active:scale-95 transition-all cursor-pointer"
                        >
